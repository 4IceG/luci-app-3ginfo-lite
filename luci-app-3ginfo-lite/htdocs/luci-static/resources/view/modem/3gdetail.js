...
function SIMdata(data) {
    ...
    // Безопасная проверка на существование simslot перед проверкой length
    if (sdata.simslot && sdata.simslot.length > 0) {
        return ui.itemlist(E('span'),[_('SIM Slot'), sdata.simslot, _('SIM IMSI'), sdata.imsi, _('SIM ICCID'), sdata.iccid, _('Modem IMEI'), sdata.imei, _('Hint'), _('CLICK ME TO SEE NEW MENU')]);
    } else {
        return ui.itemlist(E('span'),[_('SIM IMSI'), sdata.imsi, _('SIM ICCID'), sdata.iccid, _('Modem IMEI'), sdata.imei, _('Hint'), _('CLICK ME TO SEE NEW MENU')]);
    }
}
...
return view.extend({
    modemDialog: baseclass.extend({
        ...
        render: function(content) {
            ...
            ui.showModal(this.title, [E('div', {
                'class': 'cbi-section'
            }, [E('div', {
                'class': 'cbi-section-descr'
            }, this.description), E('div', {
                'class': 'cbi-section'
            }, E('p', {}, E('div', {
                'class': 'cbi-value'
            }, [E('p'), E('label', {
    ...
    render: function(data) {
        var m, s, o;
        active_select();
        var upModemDialog = new this.modemDialog(_('Defined modems'), _('Interface for selecting user defined modems.'), );
        var upSIMDialog = new this.simDialog(_('SIM card menu'), _('Information read from the SIM card and device.'), );
        if (data != null) {
            try {
          ...
          3. Port for communication with the modem.</li><li><ul>')), 'info');
           ...
           2. Check that the correct interface assigned to the modem is selected. The default name of the interface in the package is wan.</li><li><ul>')), 'info');
                        ...
                        pollData: poll.add(function() {
                            return L.resolveDefault(fs.exec_direct('/usr/share/3ginfo-lite/3ginfo.sh', 'json')).then(function(res) {
                                var json = JSON.parse(res);
                                if (!json.cport.includes('192.')) {
                                    if (json.signal == '0' || json.signal == '') {
                                        fs.exec('sleep 3');
                                        if (json.signal == '0' || json.signal == '' || json.signal == '-') {
                                            L.ui.showModal(_('3ginfo-lite'), [E('p', {
                                                'class': 'spinning'
        ...
        s.render = L.bind(function(view, section_id) {
            return E('div', {
            ...
            }, _('☰')), ]), E('h4', {}, [_('General Information')]), E('table', {
                'class': 'table'
            }, [E('tr', {
                'class': 'tr'
            }, [E('td', {
                ...
                'class': 'td left',
                'id': 'mode'
            }, ['-']), ]), ]), E('h4', {}, [_('Modem Information')]), E('table', {
                'class': 'table'
            }, [E('tr', {
                'class': 'tr'
            }, [E('td', {
                ...
                'class': 'td left',
                'width': '33%'
            }, [_('IP adress / Communication Port')]), E('td', {
                'class': 'td left',
                'id': 'cport'
            }, ['-']), ]), E('tr', {
                'class': 'tr'
                ...
                'class': 'td left',
                'width': '33%'
            }, [_('Chip Temperature')]), E('td', {
                'class': 'td left',
                'id': 'temp'
            }, ['-']), ]), ]), E('h4', {}, [_('Cell / Signal Information')]), E('table', {
                'class': 'table'
            }, [E('tr', {
                'class': 'tr'
            }, [E('td', {
                ...
                'style': 'text-align:left;font-size:66%'
            }, [_('(Received Signal Strength Indicator)')]), ]), E('td', {
                'class': 'td'
            }, E('div', {
                'id': 'rssi',
                'class': 'cbi-progressbar',
                ...
                'style': 'text-align:left;font-size:66%'
            }, [_('(Reference Signal Receive Power)')]), ]), E('td', {
                'class': 'td'
            }, E('div', {
                'id': 'rsrp',
                'class': 'cbi-progressbar',
                ...
                'style': 'text-align:left;font-size:66%'
            }, [_('(Signal to Interference plus Noise Ratio)')]), ]), E('td', {
                'class': 'td'
            }, E('div', {
                'id': 'sinr',
                'class': 'cbi-progressbar',
                ...
                'style': 'text-align:left;font-size:66%'
            }, [_('(Reference Signal Received Quality)')]), ]), E('td', {
                'class': 'td'
            }, E('div', {
                'id': 'rsrq',
                'class': 'cbi-progressbar',
                ...
                'class': 'td left',
                'width': '33%'
            }, [_('Primary band (PCC) | PCI & EARFCN')]), E('td', {
                'class': 'td left',
                'id': 'pband'
            }, ['-']), ]), E('tr', {
                'class': 'tr'
        ...
        o.onclick = function() {
            return uci.load('3ginfo').then(function() {
                ...
                // Проверяем, существует ли настройка (защита от uci failed)
                if (!searchsite) {
                    ui.addNotification(null, E('p', _('Search URL is not configured in 3ginfo.')), 'error');
                    return;
                }
                
                // Приводим все свойства к строкам (защита от undefined.length)
                var cid_dec = (json && json.cid_dec) ? String(json.cid_dec) : "";
                var cid_hex = (json && json.cid_hex) ? String(json.cid_hex) : "";
                var op_mnc = (json && json.operator_mnc) ? String(json.operator_mnc) : "";
                var op_mcc = (json && json.operator_mcc) ? String(json.operator_mcc) : "";

                if (searchsite.includes('btsearch')) {
                    var id_dec_conv = parseInt(cid_hex, 16);
                    if (cid_dec.length > 2) {
                        window.open(searchsite + cid_dec);
                    } else if (!isNaN(id_dec_conv)) {
                        window.open(searchsite + id_dec_conv);
                    }
                }
                if (searchsite.includes('lteitaly')) {
                    var first = op_mnc.slice(0, 1);
                    var second = op_mnc.slice(1, 2);
                    var zzcid = Math.round(parseInt(cid_dec || '0') / 256);
                    var cutmnc = op_mnc;
                    if (op_mnc.length === 3) {
                        if (first === '0') cutmnc = op_mnc.slice(1, 3);
                        if (first === '0' && second === '0') cutmnc = op_mnc.slice(2, 3);
                    } else if (op_mnc.length === 2) {
                        if (first === '0') cutmnc = op_mnc.slice(1, 2);
                    }
                    window.open(searchsite + op_mcc + cutmnc + '.' + zzcid);
                }
            }).catch(function(e) {
